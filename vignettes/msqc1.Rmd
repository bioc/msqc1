...
.shape_for_volcano <- function(s.8rep){
  s.8rep <- s.8rep[grep("[by]", s.8rep$Fragment.Ion), ]
  s.8rep <- s.8rep[s.8rep$Peptide.Sequence %in% peptides$Peptide.Sequence, ]
  s.8rep.peptide <- aggregate(Area ~ Peptide.Sequence + Isotope.Label.Type + instrument + File.Name.Id,
                data=s.8rep, FUN=sum)
  
  
  s.8rep.reduced.area <- data.frame(
    Peptide.Sequence=paste(s.8rep.peptide$Peptide.Sequence, s.8rep.peptide$Isotope.Label.Type, sep=','),
    Area=s.8rep.peptide$Area,
    instrument=paste(s.8rep.peptide$instrument), rep=s.8rep.peptide$File.Name.Id)
  
  
  S <- reshape(s.8rep.reduced.area,
                    v.name="Area",
  ...
  S <- data.frame(Peptide.Sequence, instrument, FC=v$fchange, p=v$pval)
}

```

```{r fig.width=5, fig.height=10, fig.retina=3, echo=TRUE}
S <- .shape_for_volcano(msqc1_8rep)

msqc1:::.figure_setup()

xyplot(-log(p, 10) ~ FC|instrument, data=S, group=Peptide.Sequence,
       panel = function(...){
